...
function insert_sanpham($iddm,$tensp,$gia,$img){
    $conn = connectdb();
    $sql = "INSERT INTO tbl_sanpham (tensp,loaisp, gia, img ) VALUES ('$tensp','$iddm', '$gia','$img')";
    $conn->exec($sql);
}
...
function getAllSPDM($loaisp){
    $conn = connectdb();
$stmt = $conn->prepare("SELECT * FROM tbl_sanpham join tbl_danhmuc on tbl_sanpham.loaisp = tbl_danhmuc.maloaisp where tbl_sanpham.loaisp = '$loaisp'");
$stmt->execute();   
// lấy dữ liệu từ bảng 
$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
//hằng số trả về kq dữ liệu là mảng
